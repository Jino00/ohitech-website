...
const LANG = {
  ...
  ja: {
    hero: {
      eyebrow: "レーザー精密加工装置専門・OHI Tech公式供給",
      headline: "Zero Heat.\nMaximum Precision.",
      sub: "ウォータージェットレーザー・微細加工・半導体貫通工程トータルソリューション",
      cta1: "技術相談のお問い合わせ",
      cta2: "デモ申し込み",
      stats: [
        { value: "1995", label: "メーカー設立" },
        { value: "30+", label: "レーザー光電子R&D（年）" },
        { value: "±2 µm", label: "最高位置決め精度" },
        { value: "0", label: "熱影響部（LML工法）" },
      ],
    },
    productNav: "製品カテゴリー",
    products: [
      {
        id: "waterjet",
        name: "ウォータージェットレーザー加工機",
        nameEn: "Waterjet Laser Machine (LML)",
        color: "bg-[var(--primary)]",
        tagline: "熱影響ゼロ・ウェハー貫通工程",
        desc: "独自特許に基づくウォータージェットレーザー加工機です。水柱が全反射でレーザーをガイドし、熱影響部（HAZ）なしにSiC・ダイヤモンド・サファイアなど第3世代半導体材料を精密に切断・ドリリングします。",
        lineup: [
          { model: "HT-WG-LC", spec: "nsグリーンレーザー 515–532nm・25/50/100W・10–50kHz" },
          { model: "ビームスポット", spec: "50–200 µm" },
          { model: "XY精度", spec: "±3 µm（オプション±2 µm）" },
          { model: "駆動軸", spec: "3/4/5軸選択" },
          { model: "装置サイズ", spec: "2,000 × 2,000 × 2,500 mm・7,000 kg" },
        ],
        apps: ["SiCパワー半導体", "ダイヤモンド基板", "サファイアウェハー", "ウェハー貫通工程（TGV）"],
        tag: "flagship",
      },
      {
        id: "fpcb",
        name: "FPCBレーザーカッティング機",
        nameEn: "Laser Cutting Machine for FPCB",
        color: "bg-[var(--primary)]",
        tagline: "冷間切断・特許コーナー工程",
        desc: "ウェアラブル・ソフトデバイス向けフレキシブル基板（FPCB）の高歩留まり冷間切断。特許コーナー工程により曲線・不規則形状も精密に加工します。",
        lineup: [
          { model: "HT-LC-FPCB", spec: "XY繰り返し精度±5 µm・Z軸±2 µm" },
          { model: "駆動方式", spec: "XYリニアモーター" },
          { model: "加工対象", spec: "PIフレキシブル基板・PET・Hard-Flex" },
          { model: "レーザー波長", spec: "IR 1064 / 532 / 355 nm UV" },
        ],
        apps: ["スマートウォッチFPCB", "イヤホン基板", "医療用ウェアラブル", "ソフトロボティクス"],
        tag: "popular",
      },
      {
        id: "etching",
        ...
        nameEn: "Laser Etching Machine for Thick Films",
        color: "bg-[var(--primary)]",
        tagline: "±2 µmエアベアリング・10ps超短パルス",
        desc: "導電性ペースト・銅めっき厚膜回路のパターニング。エアベアリング超精密ステージと10ps超短パルスレーザーで数µmの絶縁ラインを実現します。",
        lineup: [
          { model: "HT-LE-TF", spec: "XY繰り返し精度±2 µm（エアベアリング）" },
          { model: "有効ストローク", spec: "650 × 850 mm" },
          { model: "ビームスポット", spec: "6–20 µm" },
          { model: "レーザー", spec: "10ps超短パルス・多波長オプション" },
        ],
        apps: ["半導体厚膜回路", "タッチパネルITO", "MLO電極", "ディスプレイ電極"],
        tag: "",
      },
      {
        id: "industrial",
        ...
        nameEn: "Laser Engraving Machine for Industries",
        color: "bg-[var(--primary)]",
        tagline: "AOIビジョン位置決め・多波長",
        desc: "生産ライン向けシリアル番号トレーサビリティ・品質管理用の産業用マーキング装置。多波長オプションによりPCB・金属・樹脂など多様な素材に対応します。",
        lineup: [
          { model: "HT-LE-IND", spec: "IR 1064 / グリーン532 / UV 355 nm" },
          { model: "ビームスポット", spec: "10–50 µm" },
          { model: "作業ストローク", spec: "500 × 500 mm" },
          { model: "位置決め", spec: "AOIビジョン位置決め" },
        ],
        apps: ["PCBトレーサビリティ", "電子部品マーキング", "物流・EC", "医療機器ID"],
        tag: "",
      },
      {
        id: "desktop",
        ...
        nameEn: "Desktop Fiber Laser Engraving Module",
        color: "bg-[var(--primary)]",
        tagline: "小型・可搬性・カスタム設計",
        desc: "小規模工房・スタジオ・教育機関向けのコンパクトなレーザーマーキングモジュール。可搬性と設計の自由度を最適化したエントリークラスの装置です。",
        lineup: [
          { model: "HT-LE-DT", spec: "IR 1064 nm · 30W / 50W" },
          { model: "ビームスポット", spec: "30–50 µm" },
          { model: "作業範囲", spec: "20×20〜160×160 mmオプション" },
          { model: "Z軸調整", spec: "200 mm" },
        ],
        apps: ["教育・R&D", "工房・スタジオ", "小ロットカスタム製造", "プロトタイプ"],
        tag: "",
      },
      {
        id: "encoder",
        ...
        nameEn: "Optical Encoder Scales",
        color: "bg-[var(--primary)]",
        tagline: "サブミクロン精度・大量カスタム生産",
        desc: "回転運動・直線運動の位置制御用ディスク・ドラム・リニアスケール。精密レーザー加工技術を結集したサブミクロン精密測定素子です。",
        lineup: [
          { model: "HT-ENC Disc", spec: "回転位置制御・ディスクタイプ" },
          { model: "HT-ENC Drum", spec: "高速回転・ドラムタイプ" },
          { model: "HT-ENC Linear", spec: "直線運動・リニアスケール" },
          { model: "生産規模", spec: "数千〜数百万個/年のカスタム対応" },
        ],
        apps: ["工作機械", "自動車モーター", "ロボットアーム", "医療精密機器"],
        tag: "",
      },
    ],
    lmlTitle: "Laser MicroJet（LML）技術",
    lmlSub: "水がレーザーを導く革新 — HAZゼロ、無欠陥の精密加工",
    lmlPrinciple: {
      title: "動作原理",
      steps: [
        { no: "01", title: "高圧水流の生成", desc: "50–800 barの圧力でØ20–100 µmのノズルを通し、微細な水柱を形成します。" },
        { no: "02", title: "全反射レーザーガイディング", desc: "532 nmのパルスレーザーが水と空気の境界面での全反射（TIR）現象により、水柱内部で完全にガイドされます。" },
        { no: "03", title: "冷却と加工を同時に実施", desc: "水がレーザーを導くと同時に加工面をリアルタイムで冷却し、熱影響部（HAZ）を根本から遮断します。" },
        { no: "04", title: "無欠陥切断", desc: "平行な切断面、バリなし、粒子残留なし。作業距離5–50 mm、加工物厚さ0.01–30 mmに対応します。" },
      ],
    },
    lmlAdvantages: [
      { title: "HAZゼロ", desc: "水冷により熱影響部が完全になくなり、SiC・ダイヤモンドなど熱に敏感な素材も損傷なく加工できます。" },
      { title: "平行切断面", desc: "従来レーザーに比べて直角に近い切断面を実現。高アスペクト比（キーフ幅20µm超）の加工が可能です。" },
      { title: "バリなし", desc: "水中で切断が完了するため、金属バリ・破片・残留粒子のないクリーンな加工面が得られます。" },
      { title: "曲線加工特許", desc: "独自特許（US 8,422,521 B2）により等エネルギーパルスとモーションを同期。不規則な曲線も精密に切断します。" },
    ],
    lmlVsConventional: {
      title: "従来レーザー vs Laser MicroJet",
      headers: ["項目", "従来レーザー", "Laser MicroJet（LML）"],
      rows: [
        ["熱影響部（HAZ）", "発生", "なし"],
        ["切断面角度", "テーパー発生", "直角（平行壁）"],
        ["バリ", "あり", "なし"],
    ...
    tgvTitle: "ウェハー貫通工程 — OHI Tech専用サービス",
    tgvDesc: "OHI TechはHT-WG-LC装置を活用し、お客様と共にウェハー貫通ホールドリリングサービスを実際に提供しています。他社では実現が難しい高精度な貫通工程を、韓国国内で直接手がけています。",
    tgvPoints: [
      { title: "TGV（Through-Glass Via）", desc: "光学ガラスICウェハーのガラス貫通ビア（TGV）ドリリング。熱影響なしに垂直貫通孔を実現します。" },
      { title: "WBD（Wafer Based Drilling）", desc: "Si・SiC・GaN・サファイアウェハーの貫通孔加工。高アスペクト比、小径、平行壁の要求条件を満たします。" },
      { title: "第3世代半導体特殊加工", desc: "SiC・GaNパワーデバイスウェハーの微細ドリリングおよび切断。従来のダイヤモンドブレードに比べチップ損傷ゼロを実現します。" },
      { title: "工程協業", desc: "OHI Techのエンジニアがお客様の工程要件を直接分析し、装置メーカーと協力して最適な工程条件を導き出します。" },
    ],
    solutionsTitle: "業界別ソリューション",
    solutionsSub: "半導体からウェアラブルまで — レーザー精密技術の実際の適用分野",
    solutions: [
      {
        id: "semiconductor",
        name: "半導体・パワーデバイス",
        color: "bg-[var(--primary)]",
        challenge: "SiC・GaNパワーデバイスの超精密切断・ドリリング",
        highlight: "HAZゼロ・チップ損傷なし・歩留まり最大化",
        products: ["HT-WG-LC (LML)", "HT-LE-TF"],
        metrics: [
          { value: "HAZ 0", label: "熱影響部" },
          { value: "±2 µm", label: "位置決め精度" },
          { value: "第3世代", label: "半導体材料対応" },
        ],
        points: [
          "SiC・GaN・サファイア・ダイヤモンドなど第3世代半導体材料をHAZなしで切断 — 熱亀裂・マイクロクラックを防止",
          "ウェハー貫通工程（TGV/WBD）：光学ガラスIC・パワーデバイスの垂直貫通ホールドリリング",
          "従来のダイヤモンドダイシングブレードに比べチップ損傷ゼロ — 破損率低減により歩留まりを最大化",
        ],
      },
      {
        id: "fpcb",
        name: "FPCB・ウェアラブル",
        color: "bg-[var(--primary)]",
        challenge: "超薄型フレキシブル基板の高歩留まり冷間切断",
        highlight: "±5 µm繰り返し精度・特許コーナー工程",
        products: ["HT-LC-FPCB"],
        metrics: [
          { value: "±5 µm", label: "XY繰り返し精度" },
          { value: "冷間", label: "切断方式" },
          { value: "特許", label: "コーナー工程" },
        ],
        points: [
          "PI・PET・Hard-Flexフレキシブル基板の冷間切断 — 熱変形・収縮のない高歩留まり量産",
          "特許コーナー工程：曲線・不規則形状も精密に切断可能 — ウェアラブルのデザイン自由度を拡大",
          "Garminスマートウォッチ、イヤホン、医療用ウェアラブルへの適用実績を保有",
        ],
      },
      {
        id: "display",
        name: "ディスプレイ・タッチパネル",
        color: "bg-[var(--primary)]",
        challenge: "ガラス基板ITOエッチング・切断および狭額縁回路",
        highlight: "デュアルステージ・超高速ラインスクライビング",
        products: ["HT-LE-TF", "HT-WG-LC"],
        metrics: [
          { value: "6 µm", label: "最小絶縁ライン幅" },
          { value: "デュアル", label: "ステージオプション" },
          { value: "0.42 mm", label: "最薄ガラス対応" },
        ],
        points: [
          "世界最大のタッチパネルメーカーTPKへ超高速マルチステージレーザーエッチング装置を供給（2009/2011）",
          "0.42 mm超薄型DITOガラスITOパターニング — Garmin向けカスタム装置を開発・納入（2015）",
          "狭額縁回路の高速スクライビング — 世界No.1タッチパネル生産ラインの安定稼働を実現",
        ],
      },
      {
        id: "packaging",
        name: "先端パッケージング・TGV",
        color: "bg-[var(--primary)]",
        challenge: "光学ガラスIC・ファンアウトパッケージの貫通ホール",
        highlight: "TGVドリリング・OHI Tech協業工程",
        products: ["HT-WG-LC (LML)"],
        metrics: [
          { value: "TGV", label: "ガラス貫通ビア" },
          { value: "垂直壁", label: "貫通孔断面" },
          { value: "HAZ 0", label: "熱損傷" },
        ],
        points: [
          "光学ガラスICウェハーのTGVドリリング — 自社開発のレーザー装置で実証を完了",
          "OHI Techがお客様と直接ウェハー貫通工程を実施 — 韓国国内唯一のサービス",
          "Fan-Out Wafer Level Package（FOWLP）、3D IC積層パッケージ向け貫通ホール加工に対応",
        ],
      },
      {
        id: "optics",
        name: "光学・精密機器",
        color: "bg-[var(--primary)]",
        challenge: "サブミクロンエンコーダスケールおよび光学素子加工",
        highlight: "サブミクロン精度・大量カスタム生産",
        products: ["HT-ENC", "HT-WG-LC"],
        metrics: [
          { value: "sub-µm", label: "エンコーダ精度" },
          { value: "数百万", label: "年間生産規模" },
          { value: "3種", label: "スケールタイプ" },
        ],
        points: [
          "工作機械・自動車モーター・ロボットアーム向けディスク/ドラム/リニアエンコーダスケールを大量カスタム生産",
          "精密レーザー加工技術を結集したサブミクロン精密測定素子 — 数千〜数百万個/年",
          "光学素子・レンズホルダーの精密ドリリングおよびパターニング — 光学装置OEMに対応",
        ],
      },
    ],
    aboutTitle: "メーカー紹介",
    ...
    aboutMission: "コアコンピタンス",
    missionText: "レーザー光学・精密機械・モーション制御を垂直統合し、お客様専用のレーザー加工ソリューションを開発します。設計から量産までワンストップで支援します。",
    aboutVision: "保有特許",
    visionText: "US 8,422,521 B2 / US 8,710,401 B2 — 等エネルギーパルスとモーションの同期技術。曲線・不規則形状のウォータージェットレーザー切断において世界唯一の特許です。",
    certTitle: "認証・実績",
    certs: [
      { name: "ISO 9001", desc: "品質マネジメントシステム" },
      { name: "台湾証券取引所上場", desc: "30年のレーザー精密加工専門" },
      { name: "米国特許", desc: "US 8,422,521 B2" },
      { name: "米国特許", desc: "US 8,710,401 B2" },
      { name: "TPK供給実績", desc: "タッチパネル世界No.1への供給" },
      { name: "Garmin供給", desc: "0.42mm DITOガラスパターニング" },
      { name: "Merck供給", desc: "ガラスパネルレーザーカッティング" },
      { name: "ITRI納入", desc: "6kW 3Dレーザークラッディング" },
    ],
    whyTitle: "Why OHI Tech",
    whys: [
      { title: "韓国国内唯一のウェハー貫通工程", desc: "OHI TechはHT-WG-LC装置により、お客様とウェハー貫通ホールドリリングを直接行います。" },
      { title: "韓国公式総代理店", desc: "公式韓国代理店として、装置導入から工程最適化、アフターサービスまでワンストップで支援します。" },
      { title: "LML技術コンサルティング", desc: "ウォータージェットレーザー加工の適合性評価、サンプルテスト、工程条件の最適化を提供します。" },
      { title: "カスタム装置構成", desc: "3/4/5軸、レーザー出力、ノズルサイズなど、お客様の工程に合わせたカスタム構成に対応します。" },
    ],
    ctaTitle: "レーザー精密加工・ウェハー貫通工程のお問い合わせ",
    ctaDesc: "適用素材・工程仕様をお知らせいただければ、最適な装置と工程条件をご案内いたします。",
    ctaBtn1: "技術相談のお問い合わせ",
    ctaBtn2: "デモ申し込み",
  },
} as const;
