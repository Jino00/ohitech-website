...
export async function generateMetadata({
  ...
  if (!article) return {};
  return buildArticleMetadata(slug, locale, `/insights/${category}/${slug}`);
}
...
export default async function CategoryArticlePage({
  ...
  // Find article — must belong to the given category
  const article = articles.find((a) => a.slug === slug && a.category === category);
  if (!article) notFound();
  ...
  const canonicalUrl = `https://www.ohitech.co.kr/insights/${category}/${slug}`;

  // 같은 카테고리 아티클 최대 3개 (현재 글 제외)
  ...
  return (
    <>
      <ArticleJsonLd slug={slug} locale={locale} urlOverride={canonicalUrl} />
      <FaqPageJsonLd slug={slug} locale={locale} />
      <BreadcrumbJsonLd
        slug={slug}
        locale={locale}
        categorySlug={category}
        categoryLabel={categoryMeta.label[locale]}
        articleTitle={title}
        articleUrlOverride={canonicalUrl}
      />
      ...
      <main className="pt-16 min-h-screen bg-[var(--bg-alt)]">
        ...
        <section className="hero-gradient py-14">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav className="text-sm text-white/50 mb-4 flex items-center gap-1 flex-wrap">
              ...
              <Link
                href={`/insights/${category}${lp(locale)}`}
        ...
        <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {relatedArticles.length > 0 && (
            <div className="mt-10">
              <h2 className="text-base font-bold text-gray-700 mb-4">{RELATED_TITLE[locale]}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedArticles.map((rel) => {
                  const relTitle = rel.title[locale] || rel.title.ko;
                  const relBody = getArticleBody(rel, locale).replace(/##|###|\*\*|`/g, "").trim();
                  const relExcerpt = relBody.slice(0, 80) + (relBody.length > 80 ? "…" : "");
                  const relChip = CATEGORY_CHIP[rel.category];
                  return (
                    <Link
                      key={rel.slug}
                      href={`/insights/${rel.category}/${rel.slug}${lp(locale)}`}
                    ...
                    >
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${relChip.color} mb-2 inline-block`}>
                        {relChip.label[locale] || relChip.label.ko}
                      </span>
                      <p className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition-colors leading-snug line-clamp-2 mb-1">
                        {relTitle}
                      </p>
                      <p className="text-xs text-gray-400 leading-relaxed line-clamp-2">{relExcerpt}</p>
                    </Link>
                  );
                })}
              </div>
