...
export const articles: Article[] = [
  {
    ...
    body: {
      ko: `어떤 레이저 장비를 선택해야 할지 막막하신가요? CO₂, 파이버, UV — 이름은 들어봤지만 실제로 어떤 차이가 있는지, 내 공정에 무엇이 맞는지 판단하기 어려우실 텐데요.

OHI Tech는 10년간 반도체·디스플레이·자동차 부품 제조사에 레이저 장비를 공급하면서 수많은 선택 실수 사례를 봤습니다. 이 글에서는 그 경험을 바탕으로 세 가지 레이저의 실질적 차이와 공정별 최적 선택 기준을 공유합니다.
...
빔 품질 M²≈1.0, 전기효율 30~40%로 금속 절단·용접 라인의 표준이 되었습니다. OHI Tech가 자동차 배터리 셀 탭 용접 라인에 공급한 파이버 레이저의 경우, CO₂ 대비 전력 소비 65% 절감, 유지보수 비용 40% 절감을 확인했습니다.
...
OHI Tech가 FPCB 커팅 라인에 공급한 UV 레이저에서 절단면 탄화 불량률이 기존 CO₂ 대비 92% 감소했습니다. OLED 패널 수리, 유리·세라믹 미세 홀 가공에 필수입니다.
...
레이저 장비 도입을 검토 중이시라면 가공 소재와 공정 조건을 알려주세요. OHI Tech 엔지니어링팀이 최적 장비를 안내해드립니다. contact@ohitech.co.kr`,
...
At 355nm, UV photons break molecular bonds photochemically (cold ablation) rather than melting material — virtually zero HAZ. In an FPCB cutting line OHI Tech equipped with UV lasers, carbonization defects fell 92% vs the previous CO₂ system.
...
OHI Tech runs pre-purchase feasibility tests on your actual material. Send us your material specs and we will confirm processability and quality before any purchase decision. Contact: contact@ohitech.co.kr`,
...
355nm紫外光子通过光化学方式分解材料（冷消融），HAZ几乎为零。OHI Tech为某FPCB切割线配备UV激光后，碳化不良率较原CO₂方案下降92%。
...
**Q3. 如何在购买前验证工艺适配性？**
OHI Tech提供购前实材料试加工服务，告知材料规格，我们将在承诺购买前确认加工可行性和品质。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    slug: "esc",
    ...
    description: {
      ko: "Lam Research Kiyo·Flex·Versys용 ESC의 동작 원리, 두 방식의 실질적 차이, OHI Tech가 적용한 재생 수리 기준과 신품 대비 40~60% 비용 절감 사례를 설명합니다.",
      en: "Operating principles of ESCs for Lam Research Kiyo, Flex, and Versys — practical differences between JR and Coulomb types, OHI Tech refurbishment criteria, and 40–60% cost reduction vs new parts.",
      zh: "Lam Research Kiyo·Flex·Versys用静电吸盘工作原理、两种方式实质差异，以及OHI Tech翻新修复标准与较新品节约40~60%成本的实际案例。",
    },
    keywords: {
    ...
    body: {
      ko: `ESC 교체 주기가 돌아올 때마다 신품 비용이 부담스러우신가요? 아니면 재생품을 써도 되는지 확신이 서지 않으시나요?

OHI Tech는 Lam Research Kiyo·Flex·Versys 장비용 ESC를 수백 건 공급·재생하면서 어떤 상태까지 재생이 가능한지, 언제 신품으로 교체해야 하는지 기준을 직접 검증했습니다. 이 글에서 그 기준을 공유합니다.

## 결론: 재생 ESC, 신품과 성능 차이 없습니다

OHI Tech 기준을 통과한 재생 ESC는 신품 대비 흡착력·절연 저항·파티클 모두 동일 스펙을 만족합니다. 비용은 신품의 40~60% 수준입니다.
...
OHI Tech가 국내 300mm 팹에 공급한 재생 ESC 케이스입니다.

- 장비: Lam Research Kiyo × 8대
- 기존 방식: 연간 ESC 전량 신품 교체
- OHI Tech 재생 적용 후: 신품 교체 건수 60% 감소
- **연간 ESC 비용 절감: 약 45%**

재생 후 6개월 수율 데이터를 추적한 결과 신품과 유의미한 차이가 없었습니다.

## 자주 묻는 질문 (FAQ)

**Q1. 재생 ESC의 보증 기간은 얼마나 되나요?**
OHI Tech 재생 ESC는 6개월 품질 보증을 제공합니다. 재생 후 성능 측정 데이터(흡착력, 절연 저항, 파티클)와 함께 품질 보증서를 발행합니다.

**Q2. ESC 재생까지 소요 기간은 얼마나 걸리나요?**
일반적으로 수령 후 10~15 영업일 이내 반납합니다. 긴급 요청 시 별도 협의가 가능합니다.
...
ESC 재생 가능 여부 확인이나 신품 견적이 필요하시면 파트 번호와 현재 상태 사진을 보내주세요. OHI Tech 엔지니어링팀이 빠르게 검토해드립니다. contact@ohitech.co.kr`,
...
OHI Tech-certified refurbished ESCs meet the same clamping force, insulation resistance, and particle specs as new parts — at 40–60% of new-part cost.
...
## Real Cost Reduction Case

OHI Tech refurbished ESCs for a domestic 300mm fab running Lam Research Kiyo × 8 tools.

- Previous practice: full new-part ESC replacement every PM cycle
- After OHI Tech refurbishment program: 60% reduction in new-part purchases
- **Annual ESC cost reduction: ~45%**

Yield data tracked over 6 months post-refurbishment showed no statistically significant difference vs new parts.

## FAQ

**Q1. What warranty comes with refurbished ESCs?**
OHI Tech provides 6-month warranty on all refurbished ESCs, with a quality certificate including measured clamping force, insulation resistance, and particle count data.

**Q2. How long does refurbishment take?**
Typically 10–15 business days from receipt. Rush processing available upon request.

**Q3. Can you supply ESCs for tools other than Kiyo, Flex, and Versys?**
Yes — AMAT, TEL, and other tool ESCs are available. Send us the tool model and ESC part number and we will confirm availability for new supply or refurbishment. Contact: contact@ohitech.co.kr`,
...
经OHI Tech标准认证的翻新ESC，吸附力、绝缘电阻、颗粒指标均与新品一致，费用仅为新品的40~60%。
...
OHI Tech为国内300mm晶圆厂（Lam Research Kiyo×8台）翻新ESC。

- 原做法：每个PM周期全部更换新品ESC
- 应用OHI Tech翻新方案后：新品购买件数减少60%
- **年度ESC费用节约约45%**

翻新后6个月良率数据追踪结果显示，与新品无统计显著差异。

## 常见问题 (FAQ)

**Q1. 翻新ESC的质保期多长？**
OHI Tech翻新ESC提供6个月质保，并随附包含吸附力、绝缘电阻、颗粒数测量数据的质量保证书。

**Q2. 翻新需要多长时间？**
一般收到后10~15个工作日内交还。有急件需求可另行协商。

**Q3. 除Kiyo、Flex、Versys以外的设备ESC也能供应吗？**
可以，AMAT、TEL等其他设备ESC均可供应。告知设备型号和ESC料号，我们将确认新品供应或翻新可行性。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    slug: "wafer-carrier",
    ...
    body: {
      ko: `이송 중 파티클이 발생하거나, 어떤 캐리어를 써야 할지 기준이 없어 고민이신가요? FOUP, SMIF Pod, PFA 카세트 — 이름은 익숙하지만 어떤 공정에 무엇을 써야 하는지 헷갈리실 텐데요.

OHI Tech는 CK Plastics 공식 파트너로서 국내 주요 300mm 팹에 웨이퍼 캐리어를 공급하면서 공정별 선택 실수를 직접 확인했습니다. 이 글에서 그 기준을 정리합니다.
...
OHI Tech가 공급한 사례 중, 습식 세정 공정에서 일반 PC 카세트를 쓰다가 HF에 의한 소재 용출로 웨이퍼 오염이 발생한 고객이 PFA로 전환 후 파티클 불량 0건으로 개선된 케이스가 있습니다.
...
웨이퍼 캐리어 선택이나 긴급 공급이 필요하시면 현재 사용 중인 장비 환경을 알려주세요. contact@ohitech.co.kr`,
...
A customer OHI Tech supplied switched from standard PC cassettes to PFA in a wet clean process after HF-induced material leaching caused wafer contamination. Result: zero particle defects after the switch.
...
Generally every 500–1,000 cycles using IPA or DI water. Particle count testing after cleaning determines reuse eligibility. OHI Tech provides a cleaning procedure guide. Contact: contact@ohitech.co.kr`,
...
OHI Tech供货案例中，某客户在湿法清洗工艺使用普通PC卡匣，因HF引起材料溶出导致晶圆污染，改用PFA后颗粒不良降为零。
...
**Q3. FOUP的清洗周期和方法是什么？**
一般每500~1000次循环用IPA或DI水清洗，清洗后经颗粒计数检测确定是否可复用。OHI Tech提供清洗程序指导。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    ...
    body: {
      ko: `펌프가 갑자기 다운되면 라인 전체가 멈춥니다. 드라이 진공펌프의 예방 정비를 놓쳐서 비계획 다운타임이 발생하면 손실이 큽니다.

OHI Tech는 Edwards, Pfeiffer, Ebara 드라이 진공펌프를 공급하고 오버홀 서비스를 운영하면서 수명을 단축시키는 원인 패턴을 직접 파악했습니다. 이 글에서 그 체크리스트를 공유합니다.

## 결론: 수명을 줄이는 원인 1위는 N₂ 퍼지 부족입니다

공정 부산물이 펌프 내부에 고착되는 것이 가장 흔한 조기 고장 원인입니다. N₂ 퍼지 유량과 타이밍을 공정 레시피와 연동하는 것만으로 오버홀 주기를 20,000시간에서 35,000시간 이상으로 연장할 수 있습니다.
...
## OHI Tech 공급 사례 — N₂ 퍼지 최적화로 수명 연장

국내 CVD 공정 라인에서 Edwards 스크류 펌프를 운영 중인 고객사의 케이스입니다.

- 기존 상황: 평균 오버홀 주기 18,000시간 (업계 평균 이하)
- 문제 원인: N₂ 퍼지가 공정 종료 후 30분 뒤에 시작되는 설정 오류
- OHI Tech 조치: 퍼지 타이밍을 공정 종료 즉시로 수정 + 유량 1.5배 증가
- **결과: 다음 오버홀 주기 34,000시간 달성 (88% 연장)**
...
드라이 진공펌프 오버홀이나 신품 교체를 검토 중이시라면 현재 가동 시간과 증상을 알려주세요. 최적 방법을 안내해드립니다. contact@ohitech.co.kr`,
...
Process byproducts solidifying inside the pump is the most common cause of early failure. Synchronizing N₂ purge flow and timing with the process recipe alone can extend overhaul intervals from 20,000 hours to 35,000+ hours.
...
## OHI Tech Case — N₂ Purge Optimization Extended Service Life

A domestic CVD process line running Edwards screw pumps.

- Previous overhaul interval: 18,000 hours (below industry average)
- Root cause: N₂ purge configured to start 30 minutes after process end
- OHI Tech fix: purge starts immediately at process end + flow increased 1.5×
- **Result: next overhaul at 34,000 hours — 88% extension**
...
Yes — Pfeiffer, Ebara, and other major brands are supported for both new supply and overhaul. Share the pump model and we will confirm availability. Contact: contact@ohitech.co.kr`,
...
工艺副产物在泵内固化是最常见的早期故障原因。仅将N₂吹扫流量和时序与工艺配方同步，就能将大修周期从20,000小时延长至35,000小时以上。
...
## OHI Tech供货案例 — N₂吹扫优化延长寿命

国内CVD工艺产线使用Edwards螺杆泵的案例。

- 原大修周期：18,000小时（低于业界平均）
- 根本原因：N₂吹扫设置为工艺结束后30分钟启动
- OHI Tech处置：吹扫改为工艺结束即启动+流量提高1.5倍
- **结果：下次大修达到34,000小时（延长88%）**
...
可以，Pfeiffer、Ebara等主要品牌均可提供新品供应和大修服务。告知泵型号，我们将确认可行性。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    slug: "oring",
    ...
    body: {
      ko: `O-ring 하나 잘못 선택하면 챔버 가스 누설, 파티클 오염, 장비 다운타임이 발생합니다. 소재 선택 실수가 생각보다 자주 일어납니다.

OHI Tech는 Viton, Kalrez, Chemraz O-ring을 공급하면서 어떤 공정에서 어떤 소재 실수가 발생하는지 직접 확인했습니다. 이 글에서 그 기준을 공유합니다.
...
O-ring 소재 선택이나 긴급 공급이 필요하시면 공정 환경과 치수를 알려주세요. contact@ohitech.co.kr`,
...
Yes — single-piece orders and rush supply are both possible. Share the dimensions (ID × OD × CS) and material grade and we will confirm stock immediately. Contact: contact@ohitech.co.kr`,
...
**Q3. 能接受小批量和急单吗？**
单件起订和急单均可。告知尺寸（ID×OD×CS）和材料等级，我们将即时确认库存。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    ...
    updatedAt: new Date("2026-04-26"),
    title: {
      ko: "AI 서버 GPU 700W 방열 해결 — TIM·베이퍼챔버·히트파이프 소재 선택 기준",
      en: "Solving AI Server GPU 700W Thermal Management — TIM, Vapor Chamber & Heat Pipe Selection",
      zh: "AI服务器GPU 700W散热解决方案 — TIM·均热板·热管选材标准",
    },
    description: {
      ko: "NVIDIA H100/H200 GPU 700W 방열 문제 해결을 위한 TIM·베이퍼챔버·히트파이프 선택 기준, EV 배터리 방열, T-Global NMVC™ 등 OHI Tech 공급 솔루션을 설명합니다.",
      en: "TIM, vapor chamber, and heat pipe selection for NVIDIA H100/H200 GPU 700W thermal challenges, EV battery cooling, and OHI Tech T-Global NMVC™ supply solutions.",
      zh: "针对NVIDIA H100/H200 GPU 700W散热难题的TIM、均热板、热管选型标准，EV电池散热方案，以及OHI Tech供应T-Global NMVC™等解决方案。",
    },
    keywords: {
    ...
    body: {
      ko: `AI 서버 GPU 한 장이 700W를 소비하는 시대입니다. 기존 공냉 방식으로는 랙당 50kW+ 발열을 처리하기 어려워졌고, TIM 소재 선택이 시스템 성능과 수명을 직접 결정합니다.

OHI Tech는 T-Global 한국 공식 파트너로서 AI 서버, EV 배터리, 반도체 패키지에 맞는 열관리 소재를 공급하면서 어떤 소재가 어떤 환경에서 실제로 효과를 내는지 직접 확인했습니다. 이 글에서 그 기준을 공유합니다.

## 결론: AI 서버 GPU에는 10 W/m·K 이상 TIM이 필수입니다

NVIDIA H100/H200 TDP 700W에서 일반 열전도 패드(3~5 W/m·K)는 열저항이 너무 높아 GPU 온도가 설계 한계를 넘습니다. 10 W/m·K 이상 그리스 또는 인듐 포일 조합이 필요합니다.

반도체 소자는 접합부 온도(Tj)가 10°C 오를 때마다 수명이 절반으로 줄어든다는 게 업계 법칙입니다.

## TIM 타입별 특성 비교

![OHI Tech Thermal Interface Materials — T-Global 한국 공식 파트너, AI 서버·EV 배터리 열관리 솔루션](/images/insights/thermal-material-detail.png)

| TIM 타입 | 열전도율 | 두께 | 주요 용도 |
|---------|---------|------|---------|
| 열전도 그리스 | 4~10 W/m·K | 0.1~0.3mm | CPU/GPU, 고출력 소자 |
| 위상변화소재(PCM) | 3~6 W/m·K | 0.1~0.2mm | 소형 패키지, 자동화 조립 |
| 열전도 패드 | 3~15 W/m·K | 0.2~3mm | 전력 소자, EV 배터리 |
| 열전도 테이프 | 1~3 W/m·K | 0.1~0.25mm | LED, 전원 모듈 |
| 인듐 포일 | 70~80 W/m·K | 0.1~0.5mm | 고출력 반도체, AI 서버 |

## AI 서버 열관리 — 700W 문제 해결

랙당 50kW 이상의 발열을 처리하려면 직접 액냉(Direct Liquid Cooling)과 고성능 TIM의 조합이 필수입니다.

**H100/H200 권장 열관리 구성:**
- GPU 패키지 TIM: 인듐 포일(70~80 W/m·K) 또는 고성능 그리스(10 W/m·K+)
- 냉각판: 구리 마이크로채널 또는 베이퍼챔버
- 랙 수준: CDU(Coolant Distribution Unit) + 직접 액냉 루프

T-Global NMVC™(Non-Metal Vapor Chamber)는 비금속 소재로 절연성을 유지하면서 Kxy≈2,500 W/m·K의 열전도율을 구현합니다. AI 엣지 서버와 고밀도 패키지에 최적입니다.

## EV 배터리 열관리 — 급속 충전 발열 대응

리튬이온 배터리의 최적 작동 온도는 20~40°C입니다. 350kW 급속 충전 시 셀 발열이 급격히 증가하므로 셀 간 TIM 패드 설계가 핵심입니다.

**EV 배터리 모듈 권장 소재:**
- 셀 간 패드: 압축성 열전도 패드 2~6 W/m·K (T-Global Vapor-Pad™)
- 셀-냉각플레이트 계면: 위상변화소재(PCM) 또는 열전도 에폭시
- 모듈 하우징: 열전도 갭 필러

[이미지: AI서버_EV배터리_열관리_소재_적용_위치]

## 반도체 패키지 열관리

2nm 이하 공정에서 칩 전력 밀도가 100W/cm² 이상으로 상승했습니다. 패키지 레벨 TIM 선택이 클락 속도와 수명을 결정합니다.

- Die-to-Lid TIM: 인듐 포일 또는 고성능 그리스
- Lid-to-Heatsink TIM: 위상변화소재 또는 열전도 패드

## 자주 묻는 질문 (FAQ)

**Q1. 그리스와 인듐 포일 중 어느 쪽이 성능이 더 좋나요?**
열전도율은 인듐 포일(70~80 W/m·K)이 압도적으로 높습니다. 다만 조립·재작업이 어렵고 비용이 높아 양산보다는 고성능 서버에 적합합니다. 그리스는 재작업이 쉽고 비용이 낮아 대량 조립 라인에서 많이 씁니다.

**Q2. T-Global 소재는 국내에서 바로 구매 가능한가요?**
OHI Tech가 T-Global 한국 공식 파트너로 국내 재고를 보유하고 있습니다. TIM 패드, NMVC™, Vapor-Pad™, 히트파이프 등 전 라인을 공급합니다.

**Q3. 소재 선정 전 샘플 테스트가 가능한가요?**
가능합니다. 적용 환경(전력 밀도, 접촉 면적, 조립 방식)을 알려주시면 최적 소재 샘플을 제공하고 열저항 측정 데이터를 함께 드립니다.

AI 서버나 EV 배터리 방열 문제로 고민 중이시라면 현재 사용 중인 TIM 소재와 발열 조건을 알려주세요. OHI Tech 엔지니어링팀이 최적 소재를 추천해드립니다. contact@ohitech.co.kr`,
      en: `A single AI server GPU now consumes 700W. Conventional air cooling can no longer handle 50kW+ per rack, and TIM material selection directly determines system performance and lifespan.

OHI Tech is the official Korean partner for T-Global, supplying thermal management materials for AI servers, EV batteries, and semiconductor packages. Here is what we have confirmed works in practice.

## Bottom Line: AI Server GPUs Require TIM at 10 W/m·K or Higher

At NVIDIA H100/H200 TDP 700W, standard thermal pads (3–5 W/m·K) have too much thermal resistance — GPU junction temperatures exceed design limits. 10 W/m·K+ thermal grease or indium foil is required.

Industry rule: every 10°C rise in junction temperature (Tj) cuts semiconductor device lifespan in half.

## TIM Type Comparison

| TIM Type | Thermal Conductivity | Thickness | Primary Use |
|---------|---------------------|-----------|-------------|
| Thermal grease | 4–10 W/m·K | 0.1–0.3mm | CPU/GPU, high-power devices |
| Phase change material (PCM) | 3–6 W/m·K | 0.1–0.2mm | Small packages, automated assembly |
| Thermal pad | 3–15 W/m·K | 0.2–3mm | Power devices, EV batteries |
| Thermal tape | 1–3 W/m·K | 0.1–0.25mm | LED, power modules |
| Indium foil | 70–80 W/m·K | 0.1–0.5mm | High-power semiconductors, AI servers |

## AI Server Thermal Management — Solving the 700W Problem

Handling 50kW+ per rack requires direct liquid cooling combined with high-performance TIM.

**Recommended thermal stack for H100/H200:**
- GPU package TIM: indium foil (70–80 W/m·K) or high-performance grease (10 W/m·K+)
- Cold plate: copper microchannel or vapor chamber
- Rack level: CDU (Coolant Distribution Unit) + direct liquid cooling loop

T-Global NMVC™ (Non-Metal Vapor Chamber) achieves Kxy ≈ 2,500 W/m·K thermal conductivity while maintaining electrical insulation through non-metallic construction — ideal for AI edge servers and high-density packages.

## EV Battery Thermal Management

Optimal Li-ion battery operating temperature: 20–40°C. At 350kW fast charging, cell heat generation spikes sharply — thermal pad design between cells is the critical parameter.

**EV battery module recommendations:**
- Between cells: compressible thermal pad 2–6 W/m·K (T-Global Vapor-Pad™)
- Cell-to-cold plate: PCM or thermal epoxy
- Module housing: thermal gap filler

## Semiconductor Package Thermal Management

At sub-2nm processes, chip power density exceeds 100W/cm². Package-level TIM choice determines clock speed and device lifespan.

- Die-to-Lid TIM: indium foil or high-performance grease
- Lid-to-Heatsink TIM: PCM or thermal pad

## FAQ

**Q1. Thermal grease or indium foil — which performs better?**
Indium foil (70–80 W/m·K) has far higher conductivity. However, it is harder to rework and more expensive, making it better suited for high-performance servers than mass production. Grease is easier to rework and cheaper for high-volume assembly lines.

**Q2. Are T-Global materials available in Korea?**
OHI Tech holds domestic inventory as the official Korean T-Global partner — thermal pads, NMVC™, Vapor-Pad™, and heat pipes across the full lineup.

**Q3. Can I get samples before specifying a material?**
Yes. Share your application conditions (power density, contact area, assembly method) and OHI Tech will provide samples and thermal resistance measurement data. Contact: contact@ohitech.co.kr`,
      zh: `AI服务器单张GPU功耗已达700W。传统风冷无法应对每机架50kW以上的散热需求，TIM材料选型直接决定系统性能和寿命。

OHI Tech是T-Global韩国官方合作伙伴，为AI服务器、电动汽车电池和半导体封装提供热管理材料。以下是我们在实际应用中确认的有效方案。

## 结论：AI服务器GPU需要10 W/m·K以上的TIM

在NVIDIA H100/H200 TDP 700W下，普通导热垫（3~5 W/m·K）热阻过高，GPU结温会超出设计极限。需要10 W/m·K以上导热硅脂或铟箔的组合。

行业规律：结温(Tj)每升高10°C，半导体器件寿命减半。

## TIM类型对比

| TIM类型 | 导热系数 | 厚度 | 主要用途 |
|--------|---------|------|---------|
| 导热硅脂 | 4~10 W/m·K | 0.1~0.3mm | CPU/GPU、高功率器件 |
| 相变材料(PCM) | 3~6 W/m·K | 0.1~0.2mm | 小封装、自动化组装 |
| 导热垫 | 3~15 W/m·K | 0.2~3mm | 功率器件、电动汽车电池 |
| 导热胶带 | 1~3 W/m·K | 0.1~0.25mm | LED、电源模块 |
| 铟箔 | 70~80 W/m·K | 0.1~0.5mm | 高功率半导体、AI服务器 |

## AI服务器热管理 — 700W散热解决方案

处理每机架50kW以上发热，必须将直接液冷与高性能TIM相结合。

**H100/H200推荐热管理方案：**
- GPU封装TIM：铟箔（70~80 W/m·K）或高性能硅脂（10 W/m·K+）
- 冷板：铜微通道或均热板
- 机架级：CDU（冷却液分配单元）+直接液冷回路

T-Global NMVC™（非金属均热板）以非金属材料实现绝缘性，同时达到Kxy≈2,500 W/m·K的导热率，适用于AI边缘服务器和高密度封装。

## 电动汽车电池热管理

锂离子电池最优工作温度：20~40°C。350kW快充时电芯发热急剧增加，电芯间导热垫设计是核心。

**EV电池模组推荐材料：**
- 电芯间：可压缩导热垫2~6 W/m·K（T-Global Vapor-Pad™）
- 电芯-冷板界面：相变材料或导热环氧
- 模组外壳：导热间隙填充材料
...
**Q1. 导热硅脂和铟箔哪个性能更好？**
铟箔（70~80 W/m·K）导热率压倒性领先。但返工困难且价格高，更适合高性能服务器而非量产。硅脂返工容易、成本低，适合大批量组装产线。

**Q2. T-Global材料在韩国可以直接购买吗？**
OHI Tech作为T-Global韩国官方合作伙伴，备有国内库存，供应导热垫、NMVC™、Vapor-Pad™、热管全系列产品。

**Q3. 选材前可以获取样品测试吗？**
可以。告知应用条件（功率密度、接触面积、组装方式），OHI Tech将提供样品及热阻测量数据。联系邮箱：contact@ohitech.co.kr`,
    },
  },
  {
    slug: "ev-charging",
    ...
    body: {
      ko: `물류창고에 전기트럭을 도입했는데, 교대 근무 6시간 안에 배터리가 다 안 채워지는 문제 겪고 계신가요? 아니면 공장 주차장에 충전기를 설치하려는데 AC와 DC 중 무엇을 골라야 할지 막막하신가요?

이 글에서는 OHI Tech가 산업용·상업용 사업장에 EV 충전 인프라를 직접 구축하며 확인한 충전기 선택 기준, 실제 비용, 2026년 보조금 활용법을 공유합니다.
...
OHI Tech가 납품한 사업장 기준, 배터리 용량 100kWh 이상의 전기 트럭은 DC 150kW 이상 충전기 없이는 교대 근무 사이클(4~6시간)을 맞추기 어렵습니다.
...
2026년 기준 주요 보조금은 다음과 같습니다 (확정 금액은 환경부·지자체 공고 확인 필수):

- **AC 완속 충전기**: 기당 최대 약 200~400만원 지원
- **DC 급속 충전기**: 기당 최대 약 2,000~5,000만원 지원
- **사업장 설치 추가 인센티브**: 노후 건물, 취약지역 설치 시 가산 지원
...
Based on OHI Tech projects, electric trucks with 100+ kWh batteries cannot complete a charge within a 4–6 hour shift window without a DC 150kW or higher charger.
...
OHI Tech recommended configuration based on installed projects:
...
Key subsidies for 2026 (final amounts must be confirmed via official Ministry of Environment announcements):

- **AC chargers**: up to ~KRW 2–4M per unit
- **DC fast chargers**: up to ~KRW 20–50M per unit
- **Additional incentives**: older buildings or underserved areas qualify for higher support
...
基于OHI Tech项目经验，电池容量100kWh以上的电动卡车，若没有DC 150kW以上充电桩，在4~6小时班次间隔内无法完成充满。
...
| 项目 | 费用范围 |
|------|--------|
| AC 7kW充电桩（1台，含安装） | 1~3万元人民币 |
| AC 22kW充电桩（1台，含安装） | 3~7万元人民币 |
| DC 50kW充电桩（1台，含安装） | 20~40万元人民币 |
| DC 150kW以上（1台，含安装） | 50万~100万元人民币以上 |
| 用电容量扩容工程 | 5~50万元人民币（因现场条件而异） |
| OCPP计费管理系统 | 1~5万元人民币/年 |
