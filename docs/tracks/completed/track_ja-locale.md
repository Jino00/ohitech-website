...
**실측 20건 → 분류 필수 (전부 버그가 아니다)**
| 위치 | 판정 |
|---|---|
| `ProductList.tsx` **14건** | **진짜 버그** — zh·ja가 영어 라벨을 봄. 수정함 (커밋 `b7313d1`) |
...
**D-12 보강 — 윈도우 휴리스틱도 거짓음성을 낸다 (탐지기 3번째 실패)**
`detect_two_way.py`는 뒤 6줄 윈도우에 en/zh/ja 분기가 있으면 건너뛴다.
→ **무관한 인접 삼항이 진짜 버그를 가린다.** 797줄(라인업 배지)이 이렇게 누락됐고,
   서브에이전트가 직접 grep해서 잡아냈다(13 → 실제 14).
→ **최종 검증은 윈도우 없이 전수로**:
```python
# locale === "ko" 전체를 뽑아 같은 줄에 ja가 있는지 확인 + 멀티라인은 따로 처리
```
실측 결과: `locale === "ko"` 130건 = 단일행 110(ja 없음 6건 전부 의도된 정상) + 멀티라인 20(전부 ja 보유).

**탐지기 실패 3연속 기록 — 교훈**: grep 과소집계(D-10) → 2단 변종 미탐(D-12) → 윈도우 거짓음성(D-12 보강).
**"탐지기 0건"은 증거가 아니다. 탐지기 자체를 먼저 의심하라.**
