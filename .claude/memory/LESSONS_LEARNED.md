...
## 16. "도달 불가 코드 = 삭제 대상"이 아니다 — 숨김 장치인지 커밋 메시지로 확인 — 2026-07-17

### 🐛 이슈
`[category]/page.tsx`의 `isTeco` 분기와 `TecoJsonLd` 렌더가 redirect(line 85) 뒤에 가려져
도달 불가 상태였고, "죽은 코드니 `TecoJsonLd` ~130줄을 지우자"는 작업 지시가 들어왔다.
그대로 지웠으면 되살릴 코드를 파괴할 뻔했다. `dea007d` 커밋 메시지에
**"power-distribution 비노출: ... (TECO 계약 완료 시 복구)"** 라고 한시적 숨김임이 명시돼 있었다.
게다가 같은 redirect에 `TecoSection.tsx`(~2,900줄)도 가려져 있어, 130줄만 지우는 건
일관성도 없었다(도달 불가 기준을 적용하면 2,900줄도 대상).

### ✅ 해결
삭제 대신 사용자에게 TECO 계약 상태를 확인 → "공개" 결정 → 숨김 장치 4곳
(redirect / metadata 분기 / DB 필터 4개 / sitemap 주석) 해제. 코드는 한 줄도 안 지웠다.
부수적으로 인사이트 4개 로케일의 `/products/power-distribution` CTA가 307되던 것도 자동 복구.

### 📌 교훈
- **도달 불가 코드를 발견하면 지우기 전에 `git log -S` / 커밋 메시지로 의도를 확인한다.**
  "죽은 코드"와 "주차된 코드"는 diff상 똑같이 생겼지만 정반대의 조치를 요구한다.
- **삭제 범위의 일관성을 점검한다.** 같은 게이트에 가려진 더 큰 코드를 남겨두면서
  작은 것만 지우는 제안은 대개 전제가 틀렸다는 신호다.
- **세션 중 main이 앞서나갈 수 있다.** 이번에 "ItemList 변환이 main에 없다"고 보고했으나,
  실제로는 세션 도중 PR #6(`680669c`)이 머지되어 들어와 있었다. 브랜치 기준으로만 보고
  "없다"고 단정하지 말 것 — `git log origin/main`으로 현재 상태를 다시 볼 것. (원칙 22)
